...
def classify_line(line):
    ...
    # Label at start (.label, .*label, .^label)
    m = re.match(r'^(\.\^?\*?[A-Za-z_][A-Za-z0-9_]*)', code_part)
    if m:
        r['label'] = m.group(1)
    ...
    if not r['label']:
        ...
        if m:
            r['type'] = 'assignment'
            r['label'] = m.group(1)
            r['instruction'] = m.group(2) + '='
            r['operand'] = m.group(3).strip()
            return r
...
def format_line(c, indent=INDENT, comment_col=COMMENT_COL, comment_style=None):
    ...
    if c['type'] == 'assignment':
        code = c['label'].ljust(max(len(c['label']), 12))
